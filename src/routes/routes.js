...
// Pages
const Delaylazy = (path) => {
    lazy(() => {
        return new Promise((resolve) => setTimeout(resolve, 1000)).then(() =>
            Math.floor(Math.random() * 10) >= 4 ? import(path) : Promise.reject(new Error()),
        );
    });
};
...
const CompletedSetup = lazy(() => import('~/pages/SetupShop/CompletedSetup/CompletedSetup'));
// Public routes
const publicRoutes = [
...
const privateRoutes = [
    ...
    { path: config.routes.completedsetup, component: CompletedSetup },
